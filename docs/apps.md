...
```bash
git tag -a v1.0.0 -m "MLingo v1.0.0"
...
```text
GitHub repo -> Releases -> v1.0.0 -> Assets
```
...
В приложении можно нажать `Профиль -> Обновить задания`. Тот же импорт доступен в `Теория -> Паки заданий`.
