...
Browser / PWA
  ...
  -> Python container
       ...
       -> API: /api/*
       -> Postgres
       -> GitHub OAuth для опционального входа
```
...
MLingo уже умеет GitHub OAuth как способ входа, похожий на TensorTonic: пользователь нажимает GitHub, подтверждает доступ, а прогресс хранится в базе MLingo. Пока сервер не поднят, вход опционален и уроки не блокируются. Для базового входа не нужны права на запись в репозитории.
